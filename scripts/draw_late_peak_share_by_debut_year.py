...
CLUSTERS = BASE / "player_career_trajectory_classification_raw_elo.csv"
OUTPUT = BASE / "late_peak_share_by_debut_year.png"
DATA_OUTPUT = BASE / "late_peak_share_by_debut_year.csv"


def main() -> None:
    careers = pd.read_csv(CAREERS, usecols=["player_id", "first_season"])
    clusters = pd.read_csv(CLUSTERS, usecols=["player_id", "raw_elo_cluster"])
    players = careers.merge(clusters, on="player_id", how="inner")
        ...
        rows.append(
            {
                ...
                "late_peak_share_5y": (cohort["raw_elo_cluster"] == 3).mean(),
            }
        )
